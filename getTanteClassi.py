...
for runMode in Modes:
    ...
    if runMode == "validation":
        for x in range(len(quantity)):
            quantity[x] = int(float(quantity[x]) * 0.2)

    # Creo una sotto directory per la runMode
    subprocess.run(['mkdir', Dataset + "TanteClassi/" + runMode + '/'])
    subprocess.run(['mkdir', Dataset + "TanteClassi/" + runMode + '/image/'])
    subprocess.run(['mkdir', Dataset + "TanteClassi/" + runMode + '/annotation/'])
    subprocess.run(['mkdir', Dataset + "TanteClassi/" + runMode + '/review/'])
    ...
    for ind in range(len(classes)):
        ...
        for im in image_id:
            ...
            subprocess.run(['aws', 's3', '--no-sign-request', '--only-show-errors', 'cp', 's3://open-images-dataset/' + runMode + '/' + im + ".jpg", Dataset + runMode + '/image/' + im + ".jpg"])

            image = cv2.imread(Dataset + runMode + '/image/' + im + ".jpg")

            xml, image = Xml_formatter(current_annotations, runMode, image)

            cv2.imwrite(Dataset + runMode + '/image/' + im + ".jpg", image)

            g = open(Dataset + runMode + '/annotation/' + im + ".xml", "w", encoding="UTF-8")
            for x in xml:
                g.write(x)
                g.write("\n")
            g.close()
